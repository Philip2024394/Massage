...
interface ActivationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (activationCode: string) => Promise<string | void>;
  entityName: string;
}

export const ActivationModal: React.FC<ActivationModalProps> = ({ isOpen, onClose, onSubmit, entityName }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const activationError = await onSubmit(code);
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-sm w-full">
            <div className="bg-white px-6 pt-6 pb-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Activate Account</h3>
                <button onClick={handleClose} className="text-gray-400 hover:text-gray-600"><X className="h-6 w-6" /></button>
              </div>
              <p className="text-sm text-gray-600 mb-6">Enter an activation code for <span className="font-semibold">{entityName}</span> to set their subscription period.</p>
              <form onSubmit={handleSubmit}>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Massage-3"
                    className="w-full pl-12 pr-4 py-3 text-center bg-gray-50 border border-gray-300 rounded-lg shadow-inner focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                  />
                ...
                {error && <p className="text-red-500 text-center mt-3 text-sm font-medium">{error}</p>}
                 <button type="submit" disabled={loading || !code} className="w-full mt-6 bg-primary-500 text-white py-3 px-4 rounded-lg hover:bg-primary-600 font-medium disabled:opacity-50">
