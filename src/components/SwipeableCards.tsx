...
export const SwipeableCards: React.FC<SwipeableCardsProps> = ({ 
  ...
  const currentIndex = page % therapistIds.length;
  ...
  const paginate = (newDirection: number) => {
    handleInteraction();
    if ((newDirection > 0 && page === therapistIds.length - 1) || (newDirection < 0 && page === 0)) return;
    ...
    <div className="relative w-full max-w-md mx-auto">
      <div className="text-center mb-4">
        <span className="text-sm text-gray-600">
          {t('swipeable.counter', { currentIndex: page + 1, totalCount: therapistIds.length })}
        </span>
      </div>
      ...
      <div className="relative h-[850px] flex items-center justify-center">
        ...
        <AnimatePresence initial={false} custom={direction}>
          ...
          >
            <TherapistCard therapistId={therapistIds[currentIndex]} onWhatsAppClick={onWhatsAppClick} />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="flex justify-center items-center mt-10 w-full space-x-8">
        ...
        <button onClick={() => paginate(1)} disabled={page === therapistIds.length - 1} className={`p-4 rounded-full transition-colors shadow-md ${page === therapistIds.length - 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-primary-500 text-white hover:bg-primary-600'}`}>
