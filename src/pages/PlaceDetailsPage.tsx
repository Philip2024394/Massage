  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="mb-6">
            <img src={mainImage || 'https://via.placeholder.com/800x500'} alt={place.name} className="w-full h-64 sm:h-80 object-cover rounded-lg shadow-md mb-3 transition-all duration-300" />
            {place.galleryImageUrls.length > 0 && (
              <div className="grid grid-cols-5 gap-2">
                {[place.profileImageUrl, ...place.galleryImageUrls].filter(Boolean).slice(0, 5).map((url, index) => (
                  <img 
                    key={index} 
                    src={url} 
          ...
          <div className="flex items-center gap-4 mb-2">
            <div className="flex-shrink-0 flex items-center space-x-1">
              ...
              <span className="text-lg font-bold text-gray-800">{place.rating.toFixed(1)}</span>
              <span className="text-gray-600 text-sm">({place.reviewCount})</span>
            </div>
