...
serve(async (req) => {
  ...
  try {
    const { type, name, email, phone, city, service_areas } = await req.json()

    if (!type || !name || !email || !phone || !city) {
      throw new Error('Missing required parameters: type, name, email, phone, or city.')
    }
    ...
    return new Response(JSON.stringify({ id: data.id, type, login_code: data.login_code }), {
