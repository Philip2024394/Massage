...
serve(async (req) => {
  ...
  try {
    ...
    const FROM_EMAIL = 'notifications@2gomassage.app'

    // --- Email to Admin ---
    const adminEmailPayload = {
      from: `2Go Massage System <${FROM_EMAIL}>`,
      to: [ADMIN_EMAIL],
      subject: `New Therapist Registration: ${name}`,
      html: `
        <div style="font-family: sans-serif; font-size: 16px; line-height: 1.6;">
          <h2>New Registration Alert</h2>
          <p>A new therapist has registered on 2Go Massage and is awaiting your approval.</p>
          <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
          <p><strong>Name:</strong> ${name}</p>
          <p><strong>Email:</strong> ${email}</p>
          <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
          <p>Please log in to the admin dashboard to review and approve their profile.</p>
          <p>Thank you,<br/>The 2Go Massage System</p>
        </div>
      `,
    }
    ...
    const therapistEmailPayload = {
      ...
      to: [email],
      subject: `Welcome to 2Go Massage! Your Account is Pending Approval`,
      html: `
        <div style="font-family: sans-serif; font-size: 16px; line-height: 1.6;">
          <h2>Welcome, ${name}!</h2>
          <p>Thank you for registering with 2Go Massage. We're excited to have you join our community of professional therapists.</p>
