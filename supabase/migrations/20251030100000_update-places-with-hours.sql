-- ====================================================================
-- FINAL SCHEMA SCRIPT with Opening Hours for Places
-- Purpose: Safely rebuilds the schema to add opening hours to the places table
-- and removes the now-redundant is_online column.
...
BEGIN
    FOR i IN 1..70 LOOP
        ...
        VALUES (
            ...
            (SELECT array_agg(elem) FROM (SELECT unnest(ARRAY['services.massage.swedish', 'services.massage.deepTissue', 'services.massage.balinese']) AS elem ORDER BY random() LIMIT 2) AS s),
            (SELECT array_agg(elem) FROM (SELECT unnest(ARRAY['services.specialty.painRelief', 'services.specialty.relaxation']) AS elem ORDER BY random() LIMIT 1) AS s),
        ...
        VALUES (
            ...
            'Wellness ' || faker_name,
            faker_city,
            'Main Road ' || (floor(random() * 100 + 1))::text,
            faker_lat,
            faker_lng,
            '081' || (floor(random() * 900000000 + 100000000))::text,
            'active',
            (SELECT array_agg(elem) FROM (SELECT unnest(ARRAY['services.massage.swedish', 'services.place.facials', 'services.place.saunaSteam']) AS elem ORDER BY random() LIMIT 2) AS s),
            default_hours,
...
CREATE POLICY "Authenticated users can update their own images." ON storage.objects FOR UPDATE USING ( auth.uid() = owner );
