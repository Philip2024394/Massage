-- ====================================================================
-- FINAL SCHEMA SCRIPT with Languages for Places
-- Purpose: Safely rebuilds the schema to add a `languages` field to the places table.
...
BEGIN
    FOR i IN 1..70 LOOP
        ...
        VALUES (
            ...
            faker_city,
            '123 Fake St', faker_lat, faker_lng, '+628123456789', 'active', (random() > 0.3), 'Experienced therapist.', ARRAY['services.massage.swedish', 'services.massage.deepTissue'], ARRAY['services.specialty.painRelief'], ARRAY['English', 'Indonesian'], ARRAY['Certified'], 100000, 150000, 200000
        );
        INSERT INTO public.places (login_code, account_number, name, city, address, lat, lng, phone, status, opening_hours, services, languages, pricing_session_60, pricing_session_90, pricing_session_120)
        VALUES (
            'PL' || LPAD(i::text, 4, '0'),
            'PL-' || (1000 + i)::text,
            faker_name || ' Spa',
            faker_city,
            '456 Wellness Ave', faker_lat, faker_lng, '+628987654321', 'active', default_hours, ARRAY['services.massage.balinese', 'services.place.facials'], ARRAY['English', 'Indonesian'], 120000, 170000, 220000
        );
    END LOOP;
END $$;
-- ========= STEP 4: SECURITY =========
