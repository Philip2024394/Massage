-- ====================================================================
-- FINAL & CORRECTED DATABASE SETUP SCRIPT
-- Purpose: This script is idempotent. It safely cleans and rebuilds the 
-- entire schema, fixing the registration bug and adding auto-generated
-- account numbers.
...
DROP SEQUENCE IF EXISTS places_account_number_seq;
DROP SEQUENCE IF EXISTS therapists_account_number_seq;
...
-- 2.1: Create sequences for account numbers
CREATE SEQUENCE therapists_account_number_seq START 1001;
CREATE SEQUENCE places_account_number_seq START 1001;
...
-- 2.3: Create Therapists Table
CREATE TABLE public.therapists (
    id uuid NOT NULL PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
    account_number text NOT NULL DEFAULT ('TH-' || nextval('therapists_account_number_seq')),
    name text,
    phone text UNIQUE,
    experience integer,
    city text,
...
-- 2.4: Create Places Table
CREATE TABLE public.places (
    id uuid NOT NULL PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
    account_number text NOT NULL DEFAULT ('PL-' || nextval('places_account_number_seq')),
    name text,
    phone text UNIQUE,
    address text,
    city text,
